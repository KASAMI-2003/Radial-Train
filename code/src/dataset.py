...
class DetectDataset(Dataset):
    def __init__(self, 
                 ...
                 random_zoffset                                 = None,
                 normalize: bool                                = True,
                 read_from_npz: bool                            = True,
                 save_to_npz: bool                              = False,
                 ):
    ...
    def _read_afm(self, path):
        ...
        if self.read_from_npz and (path / "img.npz").exists():
            afm = np.load(path / "img.npz")['img'] / 255.0
            ...
            afm = np.stack(afms, axis = 0)
            
            if self.save_to_npz:
                np.savez_compressed(path / "img.npz", img = afm)
    ...
    def _read_afm_crop(self, idx):
        ...
        for i in idxs:
            ...
            if len(afms) and img.shape != afms[0].shape:
                img.resize(afms[0].shape)
                            
            afms.append(img)
            
        afm = np.stack(afms, axis = 0) # shape: (Z, Y, X), dtype: float
