...
class UNetND(nn.Module):
    ...
    def compute_loss(self, x: torch.Tensor, y):
        B, X, Y, Z, C = x.shape
        n_elems = C // 4  # 每元素 4 通道
        
        pred = x.reshape(B, X * Y * Z, n_elems, 4)
        targ = y.reshape(B, X * Y * Z, n_elems, 4)
        ...
        for e in range(n_elems):
            ...
            pw = self.pos_weight[e] if isinstance(self.pos_weight, torch.Tensor) and len(self.pos_weight) > e else self.pos_weight
            loss_c = F.binary_cross_entropy_with_logits(
                pred[..., e, 0].logit(), targ[..., e, 0],
                pos_weight=pw, reduction='mean')
            loss_xy = F.mse_loss(pred[..., e, (1, 2)][mask], targ[..., e, (1, 2)][mask])
            loss_z = F.mse_loss(pred[..., e, 3][mask], targ[..., e, 3][mask])

            loss_c_total += loss_c
            loss_xy_total += loss_xy
            loss_z_total += loss_z

        total_loss = (self.cls_weight * loss_c_total +
                      self.xy_weight * loss_xy_total +
                      self.z_weight * loss_z_total)

        return total_loss, {'conf': loss_c_total, 'xy': loss_xy_total, 'z': loss_z_total}
